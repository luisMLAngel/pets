...
const App = () => {
    ...
    const getPersonalItems = async () => {
        const response = await fetch("https://localhost:7128/api/pet?" + new URLSearchParams({ search }));
    ...
    const deletePersonalItem = async (id) => {
        ...
        const response = await fetch("https://localhost:7128/api/pet/" + id, {
            method: "DELETE",
        });
    ...
    const savePersonalItem = async (personalItem) => {
        ...
        const response = await fetch("https://localhost:7128/api/pet?", {
            method: "POST",
            headers: {
    ...
    const updatePersonalItem = async (personalItem) => {
        ...
        const response = await fetch("https://localhost:7128/api/pet/" + personalItem.Id, {
            method: "PATCH",
            headers: {
                'Content-Type': "application/json;charset=utf-8",
            },
                        ...
                        <CardHeader>
                            <div className="d-flex justify-content-between">
                                ...
                                <div className="d-flex gap-3">
                                    <Button size="sm" color="success" style={{ 'whiteSpace': 'nowrap'  }} onClick={() => setShowModal(!showModal)}>Agregar mascota</Button>
                                </div>
                            </div>
                        </CardHeader>
                        <CardBody>
